...
CardMovementEvent::CardMovementEvent(Game* game, const CardMovementData& cardMovementData)
    : GameEvent(game), m_cardMovementData(cardMovementData), mp_card(0), mp_destPocket(0), m_tick(0),
...
void CardMovementEvent::setCardAndPocket() {
    ...
    switch (m_cardMovementData.pocketTypeFrom) {
        ...
        case POCKET_HAND:
            ...
            mp_card = srcPlayer->hand()->take(srcPlayer->isLocalPlayer() ? m_cardMovementData.card.id : 0);
            break;
        case POCKET_TABLE:
            if (!srcPlayer) {
...
void CardMovementEvent::timerEvent(QTimerEvent*) {
    qreal progress = (m_time.msecsTo(QTime::currentTime()) * pixelsPerSecond / 1000) / m_length;
    if (progress >= 1) {
        stopTransition();
    } else {
...
void CardMovementEvent::stopTransition() {
    sm_timer.stop();
    int real     = m_time.msecsTo(QTime::currentTime());
    ...
    if (mp_card->cardData().id != 0 &&
        (mp_card->pocketType() == POCKET_HAND && mp_card->ownerId() != mp_game->playerId() ||
