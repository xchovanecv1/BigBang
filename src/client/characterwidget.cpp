...
CharacterWidget::CharacterWidget(QWidget* parent)
    : QWidget(parent), m_character(CHARACTER_UNKNOWN), m_lifePoints(0), mp_backCard(0), mp_characterCard(0),
      m_isAnimating(0), mp_cardWidgetFactory(0) {
...
void CharacterWidget::lifePointsChanged() {
    ...
    connect(&sm_timer, SIGNAL(timeout()), this, SLOT(onTimeout()));
...
void CharacterWidget::onTimeout() {
    qreal progress;
    if (!isEmpty()) {
        progress = (m_time.msecsTo(QTime::currentTime()) * pixelsPerSecond / 1000) / abs(m_sourceY - m_targetY);
        int currentY;
        if (progress >= 1) {
            currentY = m_targetY;
        } else {
        ...
        // qDebug() << m_time.elapsed() << ", " << pixelsPerSecond << ", " << progress << ", " << m_sourceY << ", " <<
        // m_targetY << ", " << currentY;
