...
void GameInfoData::write(QXmlStreamWriter* writer) const {
    ...
    foreach (const PlayerInfoData& playerInfo, players) { playerInfo.write(writer); }
    writer->writeEndElement();
}
...
void GameInfoListData::write(QXmlStreamWriter* writer) const {
    writer->writeStartElement(elementName);
    foreach (const GameInfoData& gameInfoData, *this) { gameInfoData.write(writer); }
    writer->writeEndElement();
}
...
void ActionUseAbilityData::read(XmlNode* node) {
    ...
    if (!node->attribute("target-player-id").isNull()) {
        ...
        type = TypeCards;
        foreach (XmlNode* cardNode, node->getChildren()) { targetCardsId.append(cardNode->attribute("id").toInt()); }
    }
}
