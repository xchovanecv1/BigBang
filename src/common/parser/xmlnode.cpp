...
XmlNode* XmlNode::createChildNode(const QStringView& elementName, const QXmlStreamAttributes& attributes) {
    if (m_isTextElement)
        return 0;
...
XmlNode* XmlNode::createChildTextNode(const QStringView& text) {
    if (m_isTextElement)
        return 0;
...
void XmlNode::debugPrint(int indent) const {
    ...
    foreach (XmlNode* child, m_children) { child->debugPrint(indent + 2); }
}
