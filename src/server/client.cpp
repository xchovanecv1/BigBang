...
Client::Client(QObject* parent, int id, QTcpSocket* socket)
    : QObject(parent), m_id(id), mp_playerCtrl(0), mp_socket(socket) {
    Q_ASSERT(m_id != 0);
    mp_parser = new Parser(this, socket);
    connect(mp_parser, SIGNAL(terminated()), this, SLOT(onParserTerminated()));
    connect(mp_parser, SIGNAL(sigQueryServerInfo(QueryResult)), this, SLOT(onQueryServerInfo(QueryResult)));
    connect(mp_parser,
            SIGNAL(sigQueryGameInfo(int, const QueryResult&)),
            this,
            SLOT(onQueryGameInfo(int, const QueryResult&)));
    connect(mp_parser,
            SIGNAL(sigQueryGameInfoList(const QueryResult&)),
            this,
            SLOT(onQueryGameInfoList(const QueryResult&)));
    connect(mp_parser,
            SIGNAL(sigActionCreateGame(const CreateGameData&, const CreatePlayerData&)),
            this,
            SLOT(onActionCreateGame(const CreateGameData&, const CreatePlayerData&)));
    connect(mp_parser,
            SIGNAL(sigActionJoinGame(int, int, const QString&, const CreatePlayerData&)),
            this,
            SLOT(onActionJoinGame(int, int, const QString&, const CreatePlayerData&)));
    connect(mp_parser, SIGNAL(sigActionLeaveGame()), this, SLOT(onActionLeaveGame()));
    connect(mp_parser, SIGNAL(sigActionStartGame()), this, SLOT(onActionStartGame()));
    connect(mp_parser, SIGNAL(sigActionDrawCard()), this, SLOT(onActionDrawCard()));
    connect(mp_parser,
            SIGNAL(sigActionPlayCard(const ActionPlayCardData&)),
            this,
            SLOT(onActionPlayCard(const ActionPlayCardData&)));
    connect(mp_parser,
            SIGNAL(sigActionUseAbility(const ActionUseAbilityData&)),
            this,
            SLOT(onActionUseAbility(const ActionUseAbilityData&)));
    connect(mp_parser, SIGNAL(sigActionEndTurn()), this, SLOT(onActionEndTurn()));
    connect(mp_parser, SIGNAL(sigActionPass()), this, SLOT(onActionPass()));
    connect(mp_parser, SIGNAL(sigActionDiscard(int)), this, SLOT(onActionDiscard(int)));
    connect(mp_parser, SIGNAL(sigActionChatMessage(const QString&)), this, SLOT(onActionChatMessage(const QString&)));
}
    ...
    try {
        switch (actionPlayCardData.type) {
            ...
            case ActionPlayCardData::PLAYCARD_HAND: {
                ...
                if (targetPlayer == 0) {
                    qDebug(qPrintable(
                        QString("[CLIENT]   Target player '%1' not exist!").arg(actionPlayCardData.targetPlayerId)));
                    // @todo feedback
                    return;
                }
...
void Client::onHandlerRegistered(const PublicGameView* publicGameView, PlayerCtrl* playerCtrl) {
    ...
    mp_parser->eventEnterGameMode(mp_publicGameView->id(), mp_publicGameView->name(),
...
void Client::onChatMessage(PublicPlayerView& publicPlayerView, const QString& message) {
    if (mp_parser == 0)
        return;
    mp_parser->eventChatMessage(publicPlayerView.id(), publicPlayerView.name(), message);
}
...
void Client::onGameSync() {
    ...
    gameSyncData.graveyard = PlayingCard::emptyCardData();
    if (mp_publicGameView->graveyardTop() != nullptr)
...
void Client::onPlayerDrawFromDeck(PublicPlayerView& player, QList<const PlayingCard*> cards, bool revealCards) {
    ...
    foreach (const PlayingCard* card, cards) {
        message.cards.append(card ? card->cardData() : PlayingCard::emptyCardData());
    }
    mp_parser->eventGameMessage(message);
...
void Client::onPlayerPlayCard(PublicPlayerView& player,
                              const PlayingCard* card,
                              const PlayingCard* target,
...
void Client::onPlayerPlayCardOnTable(PublicPlayerView& player,
                                     const PlayingCard* card,
                                     PublicPlayerView& targetPlayer) {
    if (mp_parser == 0)
        return;
...
void Client::onPlayerCheckDeck(PublicPlayerView& player,
                               const PlayingCard* checkedCard,
                               const PlayingCard* causedBy,
                               bool checkResult) {
    if (mp_parser == 0)
        return;
...
void Client::onPlayerStealCard(PublicPlayerView& player,
                               PublicPlayerView& targetPlayer,
                               PocketType pocketFrom,
                               const PlayingCard* card) {
    if (mp_parser == 0)
        return;
...
void Client::onCancelCard(PocketType pocketFrom,
                          const PlayingCard* card,
                          PublicPlayerView* targetPlayer,
                          PublicPlayerView* p) {
    if (mp_parser == 0)
