...
Game::Game(GameServer* parent, int gameId, const CreateGameData& createGameData)
    : QObject(parent), m_id(gameId), m_state(GAMESTATE_WAITINGFORPLAYERS), m_publicGameView(this),
      m_nextUnusedPlayerId(0), m_startable(0) {
...
void Game::removePlayer(Player* player) {
    ...
    if (player->isCreator() && m_state == GAMESTATE_WAITINGFORPLAYERS) {
        foreach (Player* p, m_playerList) { p->unregisterGameEventListener(); }
...
void Game::winningSituation(PlayerRole winners) {
    ...
    foreach (Player* player, m_playerList) {
        switch (winners) {
            case ROLE_SHERIFF:
                if (player->role() == ROLE_SHERIFF || player->role() == ROLE_DEPUTY)
                    player->setWinner(1);
                break;
            case ROLE_OUTLAW:
                ...
                break;
            case ROLE_RENEGADE:
                if (player->role() == ROLE_RENEGADE && player->isAlive())
                    player->setWinner(1);
                break;
            default:
                NOT_REACHED();
...
void Game::shufflePlayers() {
    ...
    foreach (Player* player, m_playerList) { m_publicPlayerList.append(&player->publicView()); }
}
...
QList<PlayerRole> Game::getRoleList() {
    ...
    while (*i != '\0') {
        switch (*i) {
            case 'S':
                res.append(ROLE_SHERIFF);
                break;
            case 'B':
                res.append(ROLE_OUTLAW);
                break;
            case 'V':
                res.append(ROLE_DEPUTY);
                break;
            case 'R':
                res.append(ROLE_RENEGADE);
                break;
        }
        ++i;
    }
