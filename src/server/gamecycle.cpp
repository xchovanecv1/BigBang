...
GameCycle::GameCycle(Game* game)
    : QObject(game), mp_game(game), m_state(GAMEPLAYSTATE_INVALID), mp_currentPlayer(0), mp_requestedPlayer(0),
      m_isCardEffect(0) {
}
...
GameContextData GameCycle::gameContextData() const {
    GameContextData res;
    res.currentPlayerId   = currentPlayer() ? currentPlayer()->id() : 0;
    res.requestedPlayerId = requestedPlayer() ? requestedPlayer()->id() : 0;
