...
GameEventManager::GameEventManager(Game* game) : mp_game(game) {
}
...
void GameEventManager::registerHandler(GameEventListener* handler, Player* player) {
    ...
    handler->onHandlerRegistered(&mp_game->publicGameView(), (player ? player->playerCtrl() : 0));
}
...
void GameEventManager::onChatMessage(Player* p, const QString& message) {
    foreach (Handler* h, m_handlers) { h->handler->onChatMessage(p->publicView(), message); }
}

void GameEventManager::onGameSync() {
    foreach (Handler* h, m_handlers) { h->handler->onGameSync(); }
}
...
void GameEventManager::onPlayerLeavedGame(Player* p) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerLeavedGame(p->publicView()); }
}

void GameEventManager::onPlayerUpdated(Player* p) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerUpdated(p->publicView()); }
}
...
void GameEventManager::onGameStarted() {
    foreach (Handler* h, m_handlers) { h->handler->onGameStarted(); }
}

void GameEventManager::onGameFinished() {
    foreach (Handler* h, m_handlers) { h->handler->onGameFinished(); }
}
...
void GameEventManager::onPlayerDrawFromGraveyard(Player* p, const PlayingCard* card, const PlayingCard* nextCard) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerDrawFromGraveyard(p->publicView(), card, nextCard); }
}

void GameEventManager::onPlayerDiscardCard(Player* p, PlayingCard* card, PocketType pocketFrom) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerDiscardCard(p->publicView(), card, pocketFrom); }
}

void GameEventManager::onPlayerPlayCard(Player* p, PlayingCard* card) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerPlayCard(p->publicView(), card); }
}
...
void GameEventManager::onPassTableCard(Player* p, PlayingCard* card, Player* targetPlayer) {
    foreach (Handler* h, m_handlers) { h->handler->onPassTableCard(p->publicView(), card, targetPlayer->publicView()); }
}

void GameEventManager::onPlayerRespondWithCard(Player* p, PlayingCard* card) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerRespondWithCard(p->publicView(), card); }
}

void GameEventManager::onPlayerPass(Player* p) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerPass(p->publicView()); }
}
...
void GameEventManager::onCancelCard(Player* targetPlayer, PocketType pocketFrom, PlayingCard* card, Player* causedBy) {
    foreach (Handler* h, m_handlers) {
        h->handler->onCancelCard(
            pocketFrom, card, targetPlayer ? &targetPlayer->publicView() : 0, causedBy ? &causedBy->publicView() : 0);
    }
}

void GameEventManager::onGameContextChange(const GameContextData& gameContextData) {
    foreach (Handler* h, m_handlers) { h->handler->onGameContextChange(gameContextData); }
}
...
void GameEventManager::onDeckRegenerate() {
    foreach (Handler* h, m_handlers) { h->handler->onDeckRegenerate(); }
}

void GameEventManager::onPlayerUseAbility(Player* player) {
    foreach (Handler* h, m_handlers) { h->handler->onPlayerUseAbility(player->publicView()); }
}
