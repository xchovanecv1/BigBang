...
public:
    ...
    inline PlayingCardType type() const {
        return m_type;
    }
    inline CardSuit suit() const {
        return m_suit;
    }
    inline CardRank rank() const {
        return m_rank;
    }
    inline Player* owner() const {
        return mp_owner;
    }
    inline PocketType pocket() const {
        return m_pocket;
    }
    inline bool isVirtual() const {
        return m_isVirtual;
    }
    inline PlayingCard* master() const {
        Q_ASSERT(mp_master != 0);
        return mp_master;
    }

    CardData cardData() const;
    static CardData emptyCardData();
...
private:
    ...
    inline void setPocket(const PocketType& pocket) {
        m_pocket = pocket;
    }
